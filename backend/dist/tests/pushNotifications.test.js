...
assertTrue(pushNotificationService_1.PushNotificationService.isExpoPushToken('ExponentPushToken[xxxxxxxxxxxxxxxxxxxxxx]'), 'Valid ExponentPushToken[...] format is accepted');
assertTrue(pushNotificationService_1.PushNotificationService.isExpoPushToken('ExpoPushToken[abcdef1234567890abcdef]'), 'Valid ExpoPushToken[...] format is accepted');
assertTrue(pushNotificationService_1.PushNotificationService.isExpoPushToken('FCM-f5j8k9l0m1n2o3p4q5r6s7t8u9v0w1x2'), 'Valid FCM token format is accepted');
assertFalse(pushNotificationService_1.PushNotificationService.isExpoPushToken(''), 'Empty token is rejected');
assertFalse(pushNotificationService_1.PushNotificationService.isExpoPushToken('short'), 'Short invalid token is rejected');
assertFalse(pushNotificationService_1.PushNotificationService.isExpoPushToken(null), 'Null token is rejected');
assertFalse(pushNotificationService_1.PushNotificationService.isExpoPushToken(undefined), 'Undefined token is rejected');
...
assertTrue(generatedNewReqMessage.title.includes('Urgent: O+ Blood Needed Near You'), 'Title matches example format: "Urgent: O+ Blood Needed Near You"');
assertTrue(generatedNewReqMessage.body.includes('Memorial Trauma Center'), 'Body contains hospital name for emergency context');
assertTrue(generatedNewReqMessage.body.includes('2 unit(s) of O+ blood'), 'Body contains units and blood group');
...
assertTrue(donorDeclinedMsg.body.includes('notifying other compatible donors'), 'Body reassures requester that matching continues');
...
async function testGracefulFailure() {
    ...
    const resEmpty = await pushNotificationService_1.PushNotificationService.sendPushNotifications([]);
    ...
    const resInvalid = await pushNotificationService_1.PushNotificationService.sendPushNotifications([
    ...
    await pushNotificationService_1.PushNotificationService.notifyEmergencyRequest([], requestData);
    console.log('  ✅ PASS: notifyEmergencyRequest with empty tokens executes safely');
    passed++;
    // Undefined requester token call does not throw
    await pushNotificationService_1.PushNotificationService.notifyDonorAccepted(undefined, 'Ahmed', requestData);
