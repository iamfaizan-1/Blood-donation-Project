...
const token = jsonwebtoken_1.default.sign({ id: mockUser.id, email: mockUser.email }, testJwtSecret, { expiresIn: '1h' });
const decoded = jsonwebtoken_1.default.verify(token, testJwtSecret);
...
const compatibleGroupsForA = (0, bloodCompatibility_1.getCompatibleDonorTypes)('A+');
const matchedDonors = candidateDonors
    .map(d => ({
    ...d,
    distanceKm: (0, distance_1.calculateDistanceKm)(hospitalLocation, d.coords),
}))
    .filter(d => compatibleGroupsForA.includes(d.bloodGroup) && d.isAvailable && d.isVerified && d.distanceKm <= bloodRequest.searchRadiusKm)
...
assertTrue(pushNotificationService_1.PushNotificationService.isExpoPushToken(donorPushToken), 'Step 11: Donor push token verified as valid Expo token');
...
let requestState = { ...bloodRequest, acceptedDonorId: null, status: 'matching' };
let acceptedDonation = null;
// Donor Zaid accepts
...
function handleLocationPermission(granted) {
    if (!granted) {
        return {
...
const emptyDonorPool = [];
const matchedEmpty = emptyDonorPool.filter(d => (0, bloodCompatibility_1.isBloodCompatible)(d.bloodGroup, 'AB-'));
...
function handleApiNetworkError(error) {
...
function handleBackendUnavailable(statusCode) {
    if (statusCode >= 500) {
        return {
...
function checkDuplicateRequest(newReq) {
    return activeRequests.some(r => r.requesterId === newReq.requesterId &&
...
const expiredToken = jsonwebtoken_1.default.sign({ id: 'u1', email: 'u1@life.org' }, testJwtSecret, { expiresIn: '-10s' });
let jwtExpiredError = false;
try {
    jsonwebtoken_1.default.verify(expiredToken, testJwtSecret);
}
catch (err) {
