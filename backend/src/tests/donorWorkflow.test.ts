...
const compatibleTypes = getCompatibleDonorTypes(requestBloodGroup);
...
const eligibleDonors = donors.filter(
  (d) =>
    isBloodCompatible(d.bloodGroup as any, requestBloodGroup) &&
    d.isAvailable &&
    d.isVerified
);
...
let bloodRequestState: SimulatedRequest = {
  id: 'REQ-1001',
  status: 'donor_found',
  acceptedDonorId: null,
};

let notificationStates: Record<string, string> = {
...
function simulateAccept(notificationId: string, donorId: string) {
...
let freshRequest: SimulatedRequest = {
  id: 'REQ-2002',
  status: 'donor_found',
  acceptedDonorId: null,
};
let freshNotifications: Record<string, string> = {
  notifA: 'pending',
  notifB: 'pending',
};

function simulateDecline(notificationId: string) {
...
function canRoleTransition(role: 'donor' | 'requester', current: string, next: string): boolean {
  if (role === 'donor') {
...
const statusHistoryLog: HistoryEntry[] = [];
function recordHistory(status: string, updatedBy: string) {
